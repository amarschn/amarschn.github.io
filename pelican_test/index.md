...
02/16/14
--------
So the past 10 days (10 days? Time is ridiculous), I implemented movement using target points and "draggable" navigation, where the user can drag across the screen and this will create, as well as ship rotation, where the user ship will always rotate towards it's next target point. This makes it relatively easy to control the ship, and keeps it from rotating like mad. I also created a basic projectile class, which uses LibGDX's useful Pool class. A Pool is a way to re-use memory for objects that show up a lot and then disappear (such as projectiles, raindrops, other things of that sort), without having to deal with garbage collector, which can cause issues on Android devices.

02/19/14
--------
The ship can now fire bullets! Woohooo! Also, I learned an interesting thing about Box2D and LibGDX, which is that Box2D has speed and size limits that can affect how your objects move. I discovered this after trying to make the bullets the ship fires go faster to no effect. The solution to this is to scale back and forth between the Box2D world and the game world. Currently my game world has 1 pixel: 1 meter relation to Box2D, and that will have to change to allow for faster movement speeds. Just another annoying bit of friction with using Box2D, although I assume that limits are necessary for the sake of simulation speed.